...
///      ETH fees: 20% protocol, 80% donated to LP.
contract RatchetHook is BaseHook, IRatchetHook, IUnlockCallback, ReentrancyGuard {
    ...
    /// @notice Minimum buy threshold: 0.5% of pool liquidity
    uint256 public constant MIN_BUY_THRESHOLD_BPS = 50;
    ...
    ///      Applies volume threshold: buy must be >= 0.5% of pool liquidity.
    function _afterSwap(
        address,
        PoolKey calldata key,
        ...
        if (isBuy && params.amountSpecified < 0) {
            ...
            if (tokenDelta > 0) {
                ...
                // Volume threshold: buy must be >= 0.5% of pool liquidity
                uint128 liquidity = poolManager.getLiquidity(key.toId());
                uint256 threshold =
                    (uint256(liquidity) * MIN_BUY_THRESHOLD_BPS) / BPS_DENOMINATOR;
                if (buyAmount < threshold) {
                    return (this.afterSwap.selector, 0);
                }
                ...
                if (sellAmount > 0) {
                    Currency tokenCurrency =
                        _tokenIsCurrency0 ? key.currency0 : key.currency1;
                    poolManager.sync(tokenCurrency);
                    IERC20(poolTokens[poolId]).safeTransfer(
                        address(poolManager), sellAmount
                    );
                    poolManager.settle();
    ...
    /// @dev Takes 20% protocol fee, donates remaining 80% to LP via pool manager.
    function routeFees(PoolKey calldata key) external nonReentrant {
        ...
        // Entire remaining 80% goes to LP
        uint256 lpEth = ethFees - protocolFee;

        // Donate LP ETH to LP via pool manager unlock callback
        if (lpEth > 0) {
            bool _tokenIsCurrency0 = tokenIsCurrency0[poolId];
            poolManager.unlock(abi.encode(key, _tokenIsCurrency0, lpEth));
        }

        emit FeesRouted(poolId, lpEth);
    }
